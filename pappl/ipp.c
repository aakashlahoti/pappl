    ...
    pappl_client_t *client)		// I - Client
{
  ...
  bool			printer_op = true;    // Printer operation or System Operation
  bool      is_scanner = false;   // To differentiate a printer from a scanner
          ...
          if (printer_op)
          {
            ...
            switch (ippGetOperation(client->request))
            {
              ...
              case IPP_OP_GET_NEXT_DOCUMENT_DATA :
                  ipp_get_next_document_data(client);
                  break;
    ...
    pappl_client_t *client)		// I - Client
{
  ...
  if ((attr = ippFindAttribute(client->request, "input-attributes", IPP_TAG_ZERO)) != NULL)
  {
    ...
    // TODO: if input-source is not provided and there is no input-source-default set
